...
def resize_pos_embed(posemb, grid_old_shape, grid_new_shape, num_extra_tokens):
    ...
    posemb_grid = F.interpolate(posemb_grid, size=(gs_h, gs_w), align_corners=False, mode="bilinear")
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, return_attention=False):
        ...
        def _inner_forward(x, return_attention):
            y = self.attn(self.norm1(x), H, W, return_attention)
            if return_attention:
                return y
        ...
        if self.with_cp and x.requires_grad:
            ...
            x = _inner_forward(x, return_attention)
            
        return x


class TIMMVisionTransformer(nn.Module): #BaseModule):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, 
        embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True,
        drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,norm_layer = nn.LayerNorm, layer_scale=True,
        embed_layer=PatchEmbed, act_layer=nn.GELU, window_attn=False, window_size=14, pretrained=None,
        with_cp=False):
        super().__init__()
        ...
        self.blocks = nn.Sequential(*[Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
    ...
    def forward_features(self, x):
        ...
        # in case change img size -> interpolate pos embed
        """
        if x.shape[1]!=self.pos_embed.shape[1]:
            print('change pos embed by interpolation')
            new_shape = x.shape[1] 
            self.pos_embed = self.pos_embed.permute(0, 2, 1)
            self.pos_embed = F.interpolate(self.pos_embed, size=new_shape)
            self.pos_embed = self.pos_embed.permute(0, 2, 1)
        """      
        x = self.pos_drop(x + self.pos_embed)
        
        for blk in self.blocks:
            x = blk(x, H, W, return_attention=False)
        x = self.norm_layer(x)
        return x
